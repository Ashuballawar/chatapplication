...
let userlist=await axios.get(`https://13.235.132.81:3000/group/userlist/${localStorage.getItem('groupid')}`,{headers:{'Authorization':token}})
...
userlist.data.forEach(element => {
    if(element.Admin===true&&element.Name==='You'){
        ...
        async function makeadmin(id){
           ...
           let response=await axios.post(`https://13.235.132.81:3000/group/makeadmin/${localStorage.getItem('groupid')}`,userinfo,{headers:{'Authorization':token}})
        ...
        async function removefromgroup(id){
            event.preventDefault();
            let token=localStorage.getItem('token')
            let response=await axios.delete(`https://13.235.132.81:3000/group/delete/${localStorage.getItem('groupid')}?id=${id}`,{headers:{'Authorization':token}})
        ...
        async function usernotingroup(){
            let token=localStorage.getItem('token')
            let userlist=await axios.get(`https://13.235.132.81:3000/group/adduser/${localStorage.getItem('groupid')}`,{headers:{'Authorization':token}})
    ...
    async function AddToGroup(id){
        ...
        let response=await axios.post(`https://13.235.132.81:3000/group/addtogroup/${localStorage.getItem('groupid')}`,userinfo,{headers:{'Authorization':token}})
       if(response.status===201){
